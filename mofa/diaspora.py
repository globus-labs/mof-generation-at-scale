...
Why mofka mode is non-trivial. mochi-mofka uses Argobots under thallium: every
Driver/topic/producer/consumer call must come from the Python thread that
constructed the Driver, or it aborts with ``ABT_ERR_INV_XSTREAM`` (or deadlocks
silently inside thallium). The implementation addresses this with four
cooperating pieces:
