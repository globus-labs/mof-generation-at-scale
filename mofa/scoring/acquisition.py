...
def multi_feature(ligands,stability_weight=1.0, tani_weight=0.0, sascore_weight=0.0):
    stability_ordered_inds = np.argsort([stability_weight*lig.metadata['pred_stability'] + \
                    ...
                    tani_weight*lig.hmof_tani_sim for lig in ligands])[::-1]
    return stability_ordered_inds
...
#class SuperConfig:
#   function_kwargs: dict[str, float] = {'stability_weight':1.0, 'tani_weight':0.0, 'sascore_weight':0.0}
#   function: callable[[list], list[int]] = multi_feature
#config = SuperConfig(function_kwargs={'a': 1.})
#score_fun = partial(multi_feature, **config.function_kwargs)
