...
if __name__ == '__main__':
    ...
    #     [x.result() for x in as_completed([executor.submit(preprocessing, x) for x in cif_names])]
    ...
    #     [x.result() for x in as_completed([executor.submit(run_lmp_simulation, x) for x in input_dicts])]
    # from multiprocessing.pool import ThreadPool
    # with ThreadPool(Npool) as mpool:
    ...
    lmp_jobs = [os.path.join(lmp_dir, x) for x in os.listdir(lmp_dir) if os.path.isdir(os.path.join(lmp_dir, x))]
    input_dicts = [{"Ncpus_per_job": Ncpus_per_job, 
                    "lmp_job_path": lmp_job} for lmp_job in lmp_jobs if "relaxing.2100000.restart" in os.listdir(lmp_job)]
    ...
    result.to_csv("../newMOFs_20230518_lmp/lmp_summary.csv")
    # with multiprocessing.Pool(Npool) as mpool:
    #     mpool.map_async(run_lmp_simulation, input_dicts).get()
