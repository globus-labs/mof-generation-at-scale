...
def single_conversion(cif, force_field=UFF4MOF, ff_string='UFF4MOF', small_molecule_force_field=None,
    ...
    lammps_inputs([cif, force_field, ff_string, small_molecule_force_field, outdir, charges,
                  replication, read_cifs_pymatgen, add_molecule, small_molecule_file])
...
def run_conversion():
    ...
    parser.add_argument('--small_molecule_force_field', action='store', dest='sm_ff_string', type=str,
                        required=False, default='TraPPE', help='the force field to use for small molecules')
    parser.add_argument('--outdir', action='store', dest='outdir', type=str, required=False,
                        default='unopt_lammps_data', help='where to write the lammps inputs')
    ...
    parser.add_argument('--read_cifs_pymatgen', action='store_true', dest='read_cifs_pymatgen',
                        required=False, default=False, help='use ASE to read CIF inputs')
    parser.add_argument('--add_molecule', action='store', dest='add_molecule', required=False,
                        default=None, help='name of a molecule to write molecule file for')
    parser.add_argument('--small_molecule_file', action='store', dest='sm_file', type=str, required=False, default=None,
