...
from src.difflinker_egnn import Dynamics
...
class EDM(torch.nn.Module):
    ...
    def forward(self, x, h, node_mask, fragment_mask, linker_mask, edge_mask, context=None, property_context=None):
        ...
        eps_t_hat, eps_t_hat_control = self.dynamics.forward(
            xh=z_t,
            t=t,
            node_mask=node_mask,
        ...
        ##########################################
        ##########################################
        eps_t_hat = eps_t_hat + COEFF * eps_t_hat_control ###NEWLY ADDED!!!!
        ##########################################
        ##########################################
    ...
    @torch.no_grad()
    def sample_chain(self, x, h, node_mask, fragment_mask, linker_mask, edge_mask, context, property_context, keep_frames=None):
        n_samples = x.size(0)
        n_nodes = x.size(1)
    ...
    def sample_p_zs_given_zt_only_linker(self, s, t, z_t, node_mask, fragment_mask, linker_mask, edge_mask, context, property_context):
        ...
        # Neural net prediction.
        eps_hat, eps_hat_control = self.dynamics.forward(
            xh=z_t,
            t=t,
            node_mask=node_mask,
            linker_mask=linker_mask,
        ...
        ##########################################
        ##########################################
        eps_hat = eps_hat + COEFF * eps_hat_control ###NEWLY ADDED!!!!
        ##########################################
        ##########################################
    ...
    def sample_p_xh_given_z0_only_linker(self, z_0, node_mask, fragment_mask, linker_mask, edge_mask, context, property_context):
