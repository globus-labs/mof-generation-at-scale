...
class MOFAThinker(BaseThinker, AbstractContextManager):
    ...
    def __init__(
        ...
        if hpc_config.num_workers < 2:
            raise ValueError(
                f"There must be at least two workers. Supplied: {hpc_config}"
            )
        self.assemble_workers = max(
            1, hpc_config.num_lammps_workers // 256
        )  # Ensure we keep a steady stream of MOFs
        super().__init__(
            queues,
            ResourceCounter(
                hpc_config.num_workers + self.assemble_workers,
                task_types=["generation", "lammps", "cp2k", "assembly"],
            ),
        )
        ...
        # Set up the queues
        self.mof_queue = deque(
            maxlen=8 * self.hpc_config.num_lammps_workers
        )  # Starts empty
        ...
        self.ligand_process_queue: Queue[Result] = (
            Queue()
        )  # Ligands ready to be stored in queues
        self.ligand_assembly_queue = defaultdict(
            lambda: deque(maxlen=50 * self.hpc_config.number_inf_workers)
        )  # Starts empty

        self.post_md_queue: Queue[Result] = (
            Queue()
        )  # Holds MD results ready to be stored
        ...
        )  # Store the starting weights, which we'll always use as a starting point for training
        ...
        self.model_iteration = (
            0  # Which version of the model we used for generating a ligand
        )
        ...
        self.generate_write_lock: Lock = (
            Lock()
        )  # Two threads write to the same generation output
        for name in [
            "generation-results",
            "simulation-results",
            "training-results",
    ...
    def store_generation(self, result: Result):
        ...
        self.logger.info(
            f"Generator task type={result.method} for anchor_type={anchor_type} size={size} finished"
        )
    ...
    def process_ligands(self):
        ...
        while not (self.done.is_set() and self.queues.wait_until_done(timeout=0.01)):
            ...
            if result.success:
                ...
                self.logger.info(
                    f"Received {len(all_records)} {anchor_type} ligands of size {size} from model v{model_version}, "
                ...
                result.task_info["process_done"] = (
                    datetime.now().timestamp()
                )  # TODO (wardlt): exalearn/colmena#135
                ...
                self.ligand_assembly_queue[anchor_type].extend(
                    valid_ligands
                )  # Shoves old ligands out of the deque
                self.logger.info(
                    f"Current length of {anchor_type} queue: {len(self.ligand_assembly_queue[anchor_type])}"
                )
    ...
    def store_assembly(self, result: Result):
        ...
        if not result.success:
            self.logger.warning(
                f"Assembly task failed: {result.failure_info.exception}\nStack: {result.failure_info.traceback}"
            )
            return
        ...
        print(
            result.json(exclude={"inputs", "value"}),
            file=self._output_files["assembly-results"],
            flush=True,
        )
    ...
    def store_lammps(self, result: Result):
        ...
        if not result.success:
            ...
            self.logger.info(
                f"Successful computation. Budget remaining: {self.simulations_left}"
            )
        print(
            result.json(exclude={"inputs", "value"}),
    ...
    def process_md_results(self):
        ...
        while not (self.done.is_set() and self.queues.wait_until_done(timeout=0.01)):
            ...
            self.logger.info(
                f"Received a trajectory of {len(traj)} frames for mof={name}. Backlog: {self.post_md_queue.qsize()}"
            )
            ...
            if (
                self.num_lammps_completed >= self.trainer_config.minimum_train_size
    ...
    def retrain(self):
        ...
        while not self.done.is_set():
            ...
            if self.num_raspa_completed < self.trainer_config.minimum_train_size:
                ...
                to_include = min(
                    int(self.num_lammps_completed * self.trainer_config.best_fraction),
                    self.trainer_config.maximum_train_size,
                )
                sort_order = pymongo.ASCENDING
            else:
                sort_field = "gas_storage.CO2"
                to_include = min(
                    int(self.num_raspa_completed * self.trainer_config.best_fraction),
                    self.trainer_config.maximum_train_size,
                )
            ...
            query["structure_stability.uff"] = {
                "$lt": self.trainer_config.maximum_strain
            }
            ...
            if (len(examples) == 0 or len(examples) == last_train_size) and len(
                examples
            ) < self.trainer_config.maximum_train_size:
                self.logger.info(
                    f"The number of training examples for {sort_field} with strain below {self.trainer_config.maximum_strain:.2f}"
                ...
                continue
            self.logger.info(
                f"Gathered the top {len(examples)} with strain below {self.trainer_config.maximum_strain:.2f} records based on {sort_field}"
            )
            last_train_size = len(
                examples
            )  # So we know what the training set size was for the next iteration
            ...
            if result.success:
                ...
                self.logger.info(
                    f"Received training result. Updated generator path to {new_model_path}, version number to {self.model_iteration}"
                )
            else:
                self.logger.warning(
                    f"Training failed: {result.failure_info.exception} - {result.failure_info.traceback}"
                )
            shutil.rmtree(train_dir)  # Clear training directory when done

            print(
                result.json(exclude={"inputs", "value"}),
                file=self._output_files["training-results"],
                flush=True,
            )
    ...
    def store_cp2k(self, result: Result):
        ...
        if not result.success:
            self.logger.warning(
                f"Task {result.method} failed for {mof_name}. Exception: {result.failure_info.exception}"
            )
            ...
            self.logger.info(
                f"Completed CP2K computation for {mof_name}. Runtime: {result.time.running:.2f} s. Started partial charge computation"
            )
            ...
            self.logger.info(
                f"Partial charges are complete for {mof_name}. Submitted RASPA"
            )
            ...
            self.logger.info(
                f"Stored gas storage capacity for {mof_name}: {storage_mean:.3e} +/- {storage_std:.3e}. Completed {self.num_raspa_completed}"
            )
        else:
...
if __name__ == "__main__":
    ...
    group.add_argument(
        ...
        default=10000,
        type=int,
        help="Size threshold to use proxystore for data (bytes)",
    )

    group = parser.add_argument_group(
        title="Mini App Settings", description="Mini App and Octopus configuration"
    )
    group.add_argument(
        "--launch-option", default="both", help="require one of [both|thinker|server]"
    )
    ...
    gen_func = partial(
        run_generator, n_samples=args.num_samples, device=hpc_config.torch_device
    )
    ...
    cp2k_runner = CP2KRunner(
        cp2k_invocation=hpc_config.cp2k_cmd, run_dir=run_dir / "cp2k-runs"
    )
    cp2k_fun = partial(
        cp2k_runner.run_optimization, steps=args.dft_opt_steps
    )  # Optimizes starting from assembled structure
    update_wrapper(cp2k_fun, cp2k_runner.run_optimization)

    # Make the RASPA function
    raspa_runner = RASPARunner(raspa_sims_root_path=run_dir / "raspa-runs")
    ...
    loggers = (
        [my_logger, thinker.logger, queuelogger]
        if args.launch_option in ["both", "thinker"]
        else [my_logger, queuelogger]
    )
