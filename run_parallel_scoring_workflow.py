...
from mofa.scoring.graph import GraphFeatureScorer, LigandGraphFeatureScorer
from mofa.scoring.priority_buffer import RelativePriorityBuffer
...
class MOFAThinker(BaseThinker, AbstractContextManager):
    ...
    ligand_assembly_queue: dict[str, RelativePriorityBuffer[LigandDescription]]
    ...
    def __init__(
        ...
        super().__init__(
            queues,
            ResourceCounter(
                hpc_config.num_workers, task_types=["generation", "lammps", "cp2k"]
            ),
        )
        ...
        # NOTE (nathaniel-hudson): Part of the code that incorporates the graph-based scoring code
        #  with the goal of improving ligand diversity/representation.
        self.ligand_assembly_queue = defaultdict(
            lambda: RelativePriorityBuffer(LigandGraphFeatureScorer(n_clusters=200), maxlen=200)
        )
    ...
    def assemble_new_mofs(self):
        ...
        for anchor_type in self.generator_config.anchor_types:
            ...
            if have < self.generator_config.min_ligand_candidates:
                self.logger.info(
                    f"Too few candidate for anchor_type={anchor_type}. have={have}, need={self.generator_config.min_ligand_candidates}"
                )
                return
        ...
        while num_added < self.mofs_per_call and attempts_remaining > 0:
            ...
            requirements = {
                "COO": 2,
                "cyano": 1,
            }  # TODO (wardlt): Do not hard code this
    ...
    def retrain(self):
        ...
        to_include = min(
            int(
                ...
                * self.trainer_config.best_fraction
            ),
        ...
        self.logger.info(
            "Submitted task for execution on any worker. Waiting until complete"
        )
        ...
        if result.success:
            ...
            self.logger.info(
                f"Received training result. Updated generator path to {result.value}"
            )
        else:
            self.logger.warning(
...
if __name__ == "__main__":
    ...
    cp2k_fun = partial(
        cp2k_runner.run_single_point, structure_source=("uff", -1)
    )  # Run the last structure from the UFF traj
