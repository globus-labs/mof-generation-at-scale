...
PROPERTY_DIVISOR = 14. #14 highest?
...
def coord2diff(x, edge_index, norm_constant=1):
    # edge_index = torch.stack(edge_index, dim=0).to(x.device)
    row, col = edge_index.to(x.device) 
...
class Dynamics(nn.Module):
    def __init__(
            self, n_dims, in_node_nf, context_node_nf, property_context_node_nf=1, hidden_nf=64, device='cpu', activation=nn.SiLU(),
        ...
        self.property_context_node_nf = property_context_node_nf
        self.total_context_node_nf = context_node_nf + property_context_node_nf
        self.condition_time = condition_time
        self.model = model
        self.centering = centering

        in_node_nf = in_node_nf + context_node_nf + condition_time
        in_node_nf_control = in_node_nf + property_context_node_nf
        # print(in_node_nf, )
        if self.model == 'egnn_dynamics':
            ...
            self.dynamics_control = EGNN(
                in_node_nf=in_node_nf_control,
                in_edge_nf=1,
                hidden_nf=hidden_nf, device=device,
                activation=nn.ReLU(),
                n_layers=n_layers//3,
            ...
            self.dynamics_control = GNN(
                in_node_nf=in_node_nf_control+3,
                in_edge_nf=0,
                hidden_nf=hidden_nf,
                out_node_nf=in_node_nf_control+3,
                device=device,
                activation=activation,
                n_layers=n_layers,
                attention=attention,
        ...
        self.edge_cache = {}
        # self.copy_weights(self.dynamics, self.dynamics_control)

    def copy_weights(self, module0, module1):
        for param0, param1 in zip(module0.parameters(), module1.parameters()):
            if param0.size() == param1.size():
                param1.data.copy_(param0.data)
            else:
                continue
        print("ControlNet's weights have been copied from original!")

    def forward(self, t, xh, node_mask, linker_mask, edge_mask, context, property_context):
        """
        - t: (B)
        ...
        if property_context is not None:
            property_context = torch.clamp(property_context.view(bs*n_nodes, self.property_context_node_nf) / PROPERTY_DIVISOR, 0.1, 1.) #property_context_node_nf=1 for now... PROPERTY_DIVISOR for normalization
            property_context = property_context + torch.randn_like(property_context)
            h_c = torch.cat([h, property_context], dim=1) #ASSUME we have context as well!!!!

        # Forward EGNN
        # Output: h_final (B*N, nf), x_final (B*N, 3), vel (B*N, 3)
        if self.model == 'egnn_dynamics':
            # print("here", h.size(), x.size())
            ...
            # print("here2", h_c.size(), x.size())
            h_final_c, x_final_c = self.dynamics_control(
                h_c.clone(),
                x.clone(),
                edges,
                node_mask=node_mask,
            ...
            vel_c = (x_final_c - x) * node_mask  # This masking operation is redundant but just in case
        elif self.model == 'gnn_dynamics':
            xh = torch.cat([x, h], dim=1)
            output = self.dynamics(xh, edges, node_mask=node_mask)
            xh_c = torch.cat([x, h_c], dim=1)
            output_c = self.dynamics_control(xh_c, edges, node_mask=node_mask)
            vel = output[:, 0:3] * node_mask
            h_final = output[:, 3:]
            vel_c = output_c[:, 0:3] * node_mask
            h_final_c = output_c[:, 3:]
        else:
            raise NotImplementedError

        # Slice off context size
        # print(self.context_node_nf, self.total_context_node_nf)
        if context is not None:
            h_final = h_final[:, :-self.context_node_nf]
        if property_context is not None:
            h_final_c = h_final_c[:, :-self.total_context_node_nf]
        # print(h_final.size(), h_final_c.size(), self.condition_time)

        # Slice off last dimension which represented time.
        if self.condition_time:
            h_final = h_final[:, :-1]
            if property_context is not None:
                h_final_c = h_final_c[:, :-1]

        vel = vel.view(bs, n_nodes, -1)  # (B, N, 3)
        h_final = h_final.view(bs, n_nodes, -1)  # (B, N, D)
        vel_c = vel_c.view(bs, n_nodes, -1)  # (B, N, 3)
        h_final_c = h_final_c.view(bs, n_nodes, -1)  # (B, N, D)
        node_mask = node_mask.view(bs, n_nodes, 1)  # (B, N, 1)
        ...
        return torch.cat([vel, h_final], dim=2), torch.cat([vel_c, h_final_c], dim=2)
    ...
    def get_edges(self, n_nodes, batch_size):
        if n_nodes in self.edge_cache:
            ...
            if batch_size in edges_dic_b:
                ...
                edges = torch.stack(edges, dim=0).to(self.device)
